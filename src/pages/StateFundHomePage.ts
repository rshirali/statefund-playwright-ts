...
export class StateFundHomePage {
  ...
  header(): Locator {
    ...
    // Prefer the explicit header element, fall back to banner landmark.
    return this.page
  ...
  // --- High-signal, stable locators based on current DOM ---
  ...
  async open(path = '/') {
    ...
    // A pragmatic baseline: page should not be a blank shell.
  ...
  async expectCoreFooterSections() {
    // These section headings exist in the provided DOM.
